...
diseases_list = {15: 'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis', 14: 'Drug Reaction',
                 ...
                 0: '(vertigo) Paroxysmal Positional Vertigo', 2: 'Acne', 38: 'Urinary tract infection',
                 35: 'Psoriasis', 27: 'Impetigo'}

symptoms_list_processed = {symptom.replace('_', ' ').lower(): value for symptom, value in symptoms_list.items()}


# Function to extract information from the dataset
def information(predicted_dis):
    disease_description = description[description['Disease'] == predicted_dis]['Description']
    disease_description = disease_description.iloc[0] if not disease_description.empty else "No description available"

    disease_precautions = precautions[precautions['Disease'] == predicted_dis].iloc[:, 1:].values.flatten().tolist()
    disease_precautions = disease_precautions if disease_precautions else ["No precautions available"]

    disease_medications = medications[medications['Disease'] == predicted_dis]['Medication'].tolist()
    disease_medications = disease_medications if disease_medications else ["No medications available"]

    disease_diet = diets[diets['Disease'] == predicted_dis]['Diet'].tolist()
    disease_diet = disease_diet if disease_diet else ["No diet recommendations available"]

    disease_workout = workout[workout['disease'] == predicted_dis]['workout'].tolist()
    disease_workout = disease_workout if disease_workout else ["No workout recommendations available"]

    return disease_description, disease_precautions, disease_medications, disease_diet, disease_workout


# Function to pass user input symptoms to the model
def predicted_value(patient_symptoms):
    input_vector = np.zeros(len(symptoms_list_processed))

    for item in patient_symptoms:
        if item in symptoms_list_processed:
            input_vector[symptoms_list_processed[item]] = 1
        else:
            return None  # Handle missing symptom gracefully

    prediction = Rf.predict([input_vector])[0]
    return diseases_list.get(prediction, "Unknown Disease")


# Function to correct spelling of symptoms
def correct_spelling(symptom):
    matched_symptom, score = process.extractOne(symptom, list(symptoms_list_processed.keys()))
    return matched_symptom if score >= 80 else None
...
def predict():
    ...
    if request.method == 'POST':
        ...
        #     message = "No valid symptoms found in the database."
        #     return render_template('index.html', message=message)

        predicted_disease = predicted_value(corrected_symptoms)
        print("Predicted_disease", predicted_disease)
        if not predicted_disease or predicted_disease == "Unknown Disease":
            message = "Could not predict disease. Please check symptoms and try again."
            return render_template('index.html', message=message)

        disease_description, disease_precautions, disease_medications, disease_diet, disease_workout = information(
            predicted_disease)
        ...
        return render_template('index.html',
                               ...
                               my_precautions=disease_precautions,
                               medications=disease_medications,
                               my_diet=disease_diet,
